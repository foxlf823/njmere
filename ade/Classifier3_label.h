...
#ifndef SRC_Classifier3label_H_
#define SRC_Classifier3label_H_
...
public:
  ...
  inline void init(Options options) {
	  ...
	  _relrnn_inputsize = _nerrnn_outputsize*2+_otherDim; // birnn+label
	  ...
	  _rel_output_layer.initial(_rellabelSize, _rel_hidden_outputsize, true, 80, 2);
    	...
    	cout<<"Classifier3 label initial"<<endl;
  }
  ...
  inline dtype processRel(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  ...
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], nerprime[idx], relrnn_input_sdp_e1[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], nerprime[idx], relrnn_input_sdp_e2[i]);
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  int idx = example._idxOnSDP_E12A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], nerprimeLoss[idx], relrnn_input_sdp_e1_Loss[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], nerprimeLoss[idx], relrnn_input_sdp_e2_Loss[i]);
