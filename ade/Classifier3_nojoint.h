...
#ifndef SRC_Classifier3nojoint_H_
#define SRC_Classifier3nojoint_H_
...
public:
  ...
  inline void init(Options options) {
	  ...
	  _relrnn_inputsize = _wordDim; // birnn
	  ...
	  _rel_output_layer.initial(_rellabelSize, _rel_hidden_outputsize, true, 80, 2);
    	...
    	cout<<"Classifier3 nojoint initial"<<endl;
  }
  ...
  inline dtype processRel(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  ...
    	  concat1(wordprime[idx], relrnn_input_sdp_e1[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  concat1(wordprime[idx], relrnn_input_sdp_e2[i]);
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  int idx = example._idxOnSDP_E12A[i];
    	  unconcat1(wordprimeLoss[idx], relrnn_input_sdp_e1_Loss[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  unconcat1(wordprimeLoss[idx], relrnn_input_sdp_e2_Loss[i]);
  ...
  // assignment w1->w
  inline void concat1(Tensor<xpu, 2, dtype> w1, Tensor<xpu, 2, dtype> w) {
  ...
  // assignment w->w1
  inline void unconcat1(Tensor<xpu, 2, dtype> w1, Tensor<xpu, 2, dtype> w, bool bclear = false) {
