...
#ifndef SRC_Classifier3char_H_
#define SRC_Classifier3char_H_
...
public:
  ...
  inline void init(Options options) {
	  ...
	  _nerrnn_inputsize = _wordDim+_otherDim;//word+char
	  ...
	  _relrnn_inputsize = _nerrnn_outputsize*2; // birnn
	  _relrnn_outputsize = options.rnnHiddenSize;
	  _rel_hidden_inputsize = _relrnn_outputsize*4; // e12a+e22a+a2e1+a2e2
	  ...
	  _rel_output_layer.initial(_rellabelSize, _rel_hidden_outputsize, true, 80, 2);
    	...
    	cout<<"Classifier3 char initial"<<endl;
  }
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
			...
			if(options.dropProb != 0) {
			   ...
			   concat(wordprime[idx], cnn_output[idx], nerrnn_input[idx]);
      }
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  ...
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], relrnn_input_sdp_e1[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], relrnn_input_sdp_e2[i]);
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  int idx = example._idxOnSDP_E12A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], relrnn_input_sdp_e1_Loss[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], relrnn_input_sdp_e2_Loss[i]);
      ...
      _nerrnn_right.ComputeBackwardLoss(nerrnn_input, nerrnn_hidden_right_iy, nerrnn_hidden_right_oy, nerrnn_hidden_right_fy,
        ...
        for (int idx = 0; idx < seq_size; idx++) {
        	unconcat(wordprimeLoss[idx], cnn_output_loss[idx], nerrnn_inputLoss[idx]);
