...
#ifndef SRC_Classifier3sublabel_H_
#define SRC_Classifier3sublabel_H_
...
public:
  ...
  inline void init(Options options) {
	  ...
	  _nerrnn_inputsize = _wordDim+_otherDim+_otherDim; // word+pos+char
	  ...
	  _rel_hidden_inputsize = _relrnn_outputsize*4 + _nerrnn_outputsize*2; // e12a+e22a+a2e1+a2e2+e1+e2
	  ...
	  _rel_output_layer.initial(_rellabelSize, _rel_hidden_outputsize, true, 80, 2);
    	...
    	cout<<"Classifier3_sublabel initial"<<endl;
  }
  ...
  inline dtype processRel(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      vector < Tensor<xpu, 3, dtype> > charprime, charprimeLoss, charprimeMask;
      vector < Tensor<xpu, 2, dtype> > cnn_output, cnn_output_loss;
      vector < Tensor<xpu, 3, dtype> > cnn_kernelInputs, cnn_kernelOutputs;
      vector < Tensor<xpu, 3, dtype> > cnn_poolIndex;
      ...
      for(int i=0;i<seq_size;i++) {
    	  charprime.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _otherDim), 0.0));
    	  charprimeLoss.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _otherDim), 0.0));
    	  charprimeMask.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _otherDim), 1.0));
    	  cnn_output.push_back(NewTensor<xpu>(Shape2(1, _otherDim), 0.0));
    	  cnn_output_loss.push_back(NewTensor<xpu>(Shape2(1, _otherDim), 0.0));
    	  cnn_kernelInputs.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _cnn_char._kernelinputsize), 0.0));
    	  cnn_kernelOutputs.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _otherDim), 0.0));
    	  cnn_poolIndex.push_back(NewTensor<xpu>(Shape3(example._seq_chars[i].size(),1, _otherDim), 0.0));
      }
...
/*      nerrnn_hidden_merge = NewTensor<xpu>(Shape3(seq_size, 1, _nerrnn_outputsize*2), 0.0);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
		   ...
		   for(int i=0;i<example._seq_chars[idx].size();i++) {
			   ...
			   if(options.dropProb != 0) {
				   dropoutcol(charprimeMask[idx][i], options.dropProb);
				   charprime[idx][i] = charprime[idx][i] * charprimeMask[idx][i];
			   }
		   }

		   _cnn_char.ComputeForwardScore(charprime[idx], cnn_output[idx], cnn_kernelInputs[idx], cnn_kernelOutputs[idx], cnn_poolIndex[idx]);
...
/*      for(int i=0;i<seq_size;i++) {
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  ...
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], depprime[idx], relrnn_input_sdp_e1[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  concat(nerrnn_hidden_left[idx], nerrnn_hidden_right[idx], depprime[idx], relrnn_input_sdp_e2[i]);
      ...
      for(int i=0;i<sdp_e1_size;i++) {
    	  int idx = example._idxOnSDP_E12A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], depprimeLoss[idx], relrnn_input_sdp_e1_Loss[i]);

      }

      for(int i=0;i<sdp_e2_size;i++) {
    	  int idx = example._idxOnSDP_E22A[i];
    	  unconcat(nerrnn_hidden_leftLoss[idx], nerrnn_hidden_rightLoss[idx], depprimeLoss[idx], relrnn_input_sdp_e2_Loss[i]);

      }

      pool_backward(pool_e1_Loss, poolIndex_e1,  nerrnn_hidden_leftLoss);
      pool_backward(pool_e2_Loss, poolIndex_e2,  nerrnn_hidden_leftLoss);
...
/*      for(int i=0;i<seq_size;i++) {
      ...
      _nerrnn_right.ComputeBackwardLoss(nerrnn_input, nerrnn_hidden_right_iy, nerrnn_hidden_right_oy, nerrnn_hidden_right_fy,
        ...
        for (int idx = 0; idx < seq_size; idx++) {
        	unconcat(wordprimeLoss[idx], posprimeLoss[idx], cnn_output_loss[idx], nerrnn_inputLoss[idx]);
			...
			_cnn_char.ComputeBackwardLoss(charprime[idx], cnn_output[idx], cnn_output_loss[idx], charprimeLoss[idx], cnn_kernelInputs[idx], cnn_kernelOutputs[idx], cnn_poolIndex[idx]);
			   ...
			   for(int i=0;i<example._seq_chars[idx].size();i++) {
					if(options.dropProb != 0)
						charprimeLoss[idx][i] = charprimeLoss[idx][i] * charprimeMask[idx][i];
					_char.EmbLoss(example._seq_chars[idx][i], charprimeLoss[idx][i]);
			   }
        }
...
/*      FreeSpace(&nerrnn_hidden_merge);
  ...
  int predictRel(const Example& example, vector<dtype>& results) {
      ...
      vector < Tensor<xpu, 3, dtype> > charprime;
      vector < Tensor<xpu, 2, dtype> > cnn_output;
      vector < Tensor<xpu, 3, dtype> > cnn_kernelInputs, cnn_kernelOutputs;
      vector < Tensor<xpu, 3, dtype> > cnn_poolIndex;
...
/*      for(int i=0;i<seq_size;i++) {
  ...
  _cnn_char.updateAdaGrad(nnRegular, adaAlpha, adaEps);
    ...
    _char.updateAdaGrad(nnRegular, adaAlpha, adaEps);
  }
  ...
  void checkgradsNer(const vector<Example>& examples, int iter) {
    ...
    checkgradNer(examples, _cnn_char._kernel._W, _cnn_char._kernel._gradW, "_cnn_char._kernel._W", iter);
    checkgradNer(examples, _cnn_char._kernel._b, _cnn_char._kernel._gradb, "_cnn_char._kernel._b", iter);
    ...
    checkgradNer(examples, _char._E, _char._gradE, "_char._E", iter, _char._indexers);
  }
  ...
  void checkgradsRel(const vector<Example>& examples, int iter) {
    ...
    checkgradRel(examples, _cnn_char._kernel._W, _cnn_char._kernel._gradW, "_cnn_char._kernel._W", iter);
    checkgradRel(examples, _cnn_char._kernel._b, _cnn_char._kernel._gradb, "_cnn_char._kernel._b", iter);
    ...
    checkgradRel(examples, _char._E, _char._gradE, "_char._E", iter, _char._indexers);
  }
