...
#include "Example.h"
#include "ClassifierAtt.h"
#include "ClassifierCNN.h"
...
// use label accuracy on the development set
...
public:
  ...
  //ClassifierAtt<cpu> m_classifier;
  //ClassifierCNN<cpu> m_classifier;
#endif


  NNbb2(const Options &options):m_options(options) {
		unknownkey = "-#unknown#-";
		nullkey = "-#null#-";
	}
		...
		for(int docIdx=0;docIdx<documents.size();docIdx++) {
			...
			for(int sentIdx=0;sentIdx<doc.sents.size();sentIdx++) {
				...
				for(int tokenIdx=0;tokenIdx<sent.tokens.size();tokenIdx++) {
					...
					if(predicted == eg.nerGoldLabel) {
						metric_dev.correct_label_count++;
					} /*else {
						cout<<eg.nerGoldLabel<<" "<<predicted<<" "<<token.word<<endl;
					}*/
