...
#ifndef SRC_ClassifierCNN_H_
#define SRC_ClassifierCNN_H_
...
public:
  ...
  inline void init(Options options) {
	  ...
	  _poolType = options.poolType;
	  _cnn_left.initial(_rel_hidden_outputsize, _rel_hidden_inputsize, 0, _poolType, 0, 70);
	  _cnn_right.initial(_rel_hidden_outputsize, _rel_hidden_inputsize, 0, _poolType, 0, 90);

	  _rel_output_layer.initial(_rellabelSize, _rel_hidden_outputsize*2, true, 80, 2);




    	cout<<"ClassifierCNN initial"<<endl;
  }
  ...
  inline dtype processRel(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
	  ...
	  Tensor<xpu, 2, dtype> cnn_left_output, cnn_left_output_loss;
	  Tensor<xpu, 3, dtype> cnn_left_kernelInputs, cnn_left_kernelOutputs;
      Tensor<xpu, 3, dtype> cnn_left_poolIndex;

      Tensor<xpu, 3, dtype> cnn_left_M;
      Tensor<xpu, 2, dtype> cnn_left_omegaM, cnn_left_exp_omegaM, cnn_left_alpha, cnn_left_r;

	  Tensor<xpu, 2, dtype> cnn_right_output, cnn_right_output_loss;
	  Tensor<xpu, 3, dtype> cnn_right_kernelInputs, cnn_right_kernelOutputs;
      Tensor<xpu, 3, dtype> cnn_right_poolIndex;

      Tensor<xpu, 3, dtype> cnn_right_M;
      Tensor<xpu, 2, dtype> cnn_right_omegaM, cnn_right_exp_omegaM, cnn_right_alpha, cnn_right_r;


      Tensor<xpu, 2, dtype> output_input, output_input_loss;
	  ...
	  cnn_left_output = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
	  cnn_left_output_loss = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
	  cnn_left_kernelInputs = NewTensor<xpu>(Shape3(rel_seq_size, 1, _cnn_left._kernelinputsize), 0.0);
	  cnn_left_kernelOutputs = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), d_zero);
      cnn_left_poolIndex = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), d_zero);

      if(_poolType == 5) {
    	  cnn_left_M = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), 0.0);
		  cnn_left_omegaM = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_left_exp_omegaM = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_left_alpha = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_left_r = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
      }

	  cnn_right_output = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
	  cnn_right_output_loss = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
	  cnn_right_kernelInputs = NewTensor<xpu>(Shape3(rel_seq_size, 1, _cnn_right._kernelinputsize), 0.0);
	  cnn_right_kernelOutputs = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), d_zero);
      cnn_right_poolIndex = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), d_zero);

      if(_poolType == 5) {
    	  cnn_right_M = NewTensor<xpu>(Shape3(rel_seq_size, 1, _rel_hidden_outputsize), 0.0);
		  cnn_right_omegaM = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_right_exp_omegaM = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_right_alpha = NewTensor<xpu>(Shape2(1, rel_seq_size), 0.0);
    	  cnn_right_r = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize), 0.0);
      }

      output_input = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize*2), 0.0);
      output_input_loss = NewTensor<xpu>(Shape2(1, _rel_hidden_outputsize*2), 0.0);
      ...
      if(_poolType == 5) {
          _cnn_left.ComputeForwardScore(relrnn_hidden_left, cnn_left_output, cnn_left_kernelInputs,
        		  cnn_left_kernelOutputs, cnn_left_poolIndex, cnn_left_M, cnn_left_omegaM,
				  cnn_left_exp_omegaM, cnn_left_alpha, cnn_left_r);
          _cnn_right.ComputeForwardScore(relrnn_hidden_right, cnn_right_output, cnn_right_kernelInputs,
        		  cnn_right_kernelOutputs, cnn_right_poolIndex, cnn_right_M, cnn_right_omegaM,
				  cnn_right_exp_omegaM, cnn_right_alpha, cnn_right_r);

      } else {
          _cnn_left.ComputeForwardScore(relrnn_hidden_left, cnn_left_output, cnn_left_kernelInputs, cnn_left_kernelOutputs, cnn_left_poolIndex);
          _cnn_right.ComputeForwardScore(relrnn_hidden_right, cnn_right_output, cnn_right_kernelInputs, cnn_right_kernelOutputs, cnn_right_poolIndex);
      }

      concat(cnn_left_output, cnn_right_output, output_input);

      _rel_output_layer.ComputeForwardScore(output_input, output);

      // get delta for each output
      cost += softmax_loss(output, example._relLabels, outputLoss, _eval, example_num);

      // loss backward propagation
      _rel_output_layer.ComputeBackwardLoss(output_input, output, outputLoss, output_input_loss);

      unconcat(cnn_left_output_loss, cnn_right_output_loss, output_input_loss);

      if(_poolType == 5) {
          _cnn_left.ComputeBackwardLoss(relrnn_hidden_left, cnn_left_output, cnn_left_output_loss,
        		  relrnn_hidden_left_Loss, cnn_left_kernelInputs, cnn_left_kernelOutputs, cnn_left_poolIndex,
				  cnn_left_M, cnn_left_omegaM, cnn_left_exp_omegaM, cnn_left_alpha, cnn_left_r);
          _cnn_right.ComputeBackwardLoss(relrnn_hidden_right, cnn_right_output, cnn_right_output_loss,
        		  relrnn_hidden_right_Loss, cnn_right_kernelInputs, cnn_right_kernelOutputs, cnn_right_poolIndex,
				  cnn_right_M, cnn_right_omegaM, cnn_right_exp_omegaM, cnn_right_alpha, cnn_right_r);
      ...
      FreeSpace(&cnn_left_output);
      FreeSpace(&cnn_left_output_loss);
      FreeSpace(&cnn_left_kernelInputs);
      FreeSpace(&cnn_left_kernelOutputs);
      FreeSpace(&cnn_left_poolIndex);

      FreeSpace(&cnn_right_output);
      FreeSpace(&cnn_right_output_loss);
      FreeSpace(&cnn_right_kernelInputs);
      FreeSpace(&cnn_right_kernelOutputs);
      FreeSpace(&cnn_right_poolIndex);

      if(_poolType == 5) {
    	  FreeSpace(&cnn_left_M);
    			  FreeSpace(&cnn_left_omegaM);
    	  FreeSpace(&cnn_left_exp_omegaM);
    	  FreeSpace(&cnn_left_alpha);
    	  FreeSpace(&cnn_left_r);
    	  FreeSpace(&cnn_right_M);
    			  FreeSpace(&cnn_right_omegaM);
    	  FreeSpace(&cnn_right_exp_omegaM);
    	  FreeSpace(&cnn_right_alpha);
    	  FreeSpace(&cnn_right_r);
      }


      FreeSpace(&output_input);
      FreeSpace(&output_input_loss);
  ...
  int predictRel(const Example& example, vector<dtype>& results) {
	  ...
	  Tensor<xpu, 2, dtype> cnn_left_output;
	  Tensor<xpu, 3, dtype> cnn_left_kernelInputs, cnn_left_kernelOutputs;
      Tensor<xpu, 3, dtype> cnn_left_poolIndex;
	  ...
	  Tensor<xpu, 2, dtype> cnn_right_output;
	  Tensor<xpu, 3, dtype> cnn_right_kernelInputs, cnn_right_kernelOutputs;
      Tensor<xpu, 3, dtype> cnn_right_poolIndex;
      ...
      Tensor<xpu, 2, dtype> output_input;
...
/*		for(int i=0;i<output.size(1);i++) {
  ...
  void updateParams(dtype nnRegular, dtype adaAlpha, dtype adaEps) {
    ...
    _nerrnn_right.updateAdaGrad(nnRegular, adaAlpha, adaEps);
	  ...
	  _cnn_left.updateAdaGrad(nnRegular, adaAlpha, adaEps);
	  _cnn_right.updateAdaGrad(nnRegular, adaAlpha, adaEps);
  ...
  void checkgradsRel(const vector<Example>& examples, int iter) {
    ...
    checkgradRel(examples, _cnn_left._kernel._W, _cnn_left._kernel._gradW, "_cnn_left._kernel._W", iter);
    checkgradRel(examples, _cnn_left._kernel._b, _cnn_left._kernel._gradb, "_cnn_left._kernel._b", iter);
    if(_poolType == 5)
    	checkgradRel(examples, _cnn_left._att._omega, _cnn_left._att._gradOmega, "_cnn_left._att._omega", iter);
    checkgradRel(examples, _cnn_right._kernel._W, _cnn_right._kernel._gradW, "_cnn_right._kernel._W", iter);
    checkgradRel(examples, _cnn_right._kernel._b, _cnn_right._kernel._gradb, "_cnn_right._kernel._b", iter);
    if(_poolType == 5)
    	checkgradRel(examples, _cnn_right._att._omega, _cnn_right._att._gradOmega, "_cnn_right._att._omega", iter);
