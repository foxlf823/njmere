...
// extension to NNbb.h with an extra IMP relation
...
public:
  ...
  NNbb_imp(const Options &options):m_options(options) {
		unknownkey = "-#unknown#-";
		nullkey = "-#null#-";
	}
	...
	Prediction greedySearch(const fox::Sent & sent, Prediction& p) {
		...
		for(int tokenIdx=0;tokenIdx<sent.tokens.size();tokenIdx++) {
			...
			vector<dtype> probs;
			int bestLabel = m_classifier.predictNer(eg, probs);
